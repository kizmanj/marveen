...
$TMUX new-session -d -s "$SESSION" -c "$INSTALL_DIR/store" \
  ...
  case "$pane" in
    *"EPERM"*|*"Operation not permitted"*|*"operation not permitted"*)
      if [ "$_eperm_restarted" = "0" ]; then
        ...
        [ -e "$INSTALL_DIR/store/CLAUDE.md" ] && ln -sf "$INSTALL_DIR/store/CLAUDE.md" "$_CHANNELS_STARTDIR/CLAUDE.md" 2>/dev/null || true
